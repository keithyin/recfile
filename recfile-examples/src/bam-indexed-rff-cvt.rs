...
use recfile::io::{
    get_bincode_cfg,
    indexed_rw::{IndexedRffReader, IndexedRffWriter},
    v2,
};
...
fn enc_worker(recv: Receiver<bam::Record>, sender: Sender<(String, Vec<u8>)>) {
    ...
    for record in recv {
        ...
        let serial = bincode::encode_to_vec(&read, cfg).unwrap();
        let name = read.name.clone();
        sender.send((name, serial)).unwrap();
    }
}
...
fn decode_worker(sender: Sender<bam::Record>, recv: Receiver<Vec<u8>>) {
    let cfg = get_bincode_cfg();
    for data in recv {
        let (batch_records, _nbytes): (ReadInfo, usize) =
            bincode::decode_from_slice(&data, cfg).unwrap();
        sender.send(batch_records.to_record()).unwrap();
    }
}
...
fn bam2idxrff(cli: &Cli) {
    ...
    std::thread::scope(|thread_scope| {
        ...
        let mut writer =
            IndexedRffWriter::new_writer(&out_path, NonZero::new(cli.writer_threads).unwrap());
        let pb = get_spin_pb(format!("writing {:?}", out_path), DEFAULT_INTERVAL);
        for data in write_recv {
            let _ = unsafe { writer.write_serialized_data(&data.1, &data.0) };
            pb.inc(1);
        }
        pb.finish();
    });
...
fn indexed_rff_read(cli: &Cli) -> Vec<Vec<u8>> {
    let in_path = cli.in_path.clone();
...
fn indexed_rff_read_mmap(cli: &Cli) -> Vec<Vec<u8>> {
    let in_path = cli.in_path.clone();
...
fn indexed_rff_read_and_drop(cli: &Cli) {
    let in_path = cli.in_path.clone();
...
fn g_read_write(cli: &Cli) {
    let all_data = indexed_rff_read(cli);
    gwrite(cli, all_data);
}
...
fn main() {
    ...
    match cli.mode.as_ref() {
        "bam2idxrff" => bam2idxrff(&cli),
        "b2g2-codec" => b2g2_with_codec(&cli),
        "b2g2-codec-lz4" => b2g2_with_codec_lz4(&cli),
        "idxrff2bam" => idxrff2bam(&cli),
        "g2b2-codec" => g2b2_with_codec(&cli),

        "b2b" => b2b(&cli),
        "g2g" => g2g(&cli),
        "indexed_rff_read" => {
            indexed_rff_read(&cli);
        }
        "indexed_rff_read_mmap" => {
            indexed_rff_read_mmap(&cli);
        }
        "gread2big-buf" => {
            gread2big_buf(&cli);
        }
        "indexed_rff_read_and_drop" => indexed_rff_read_and_drop(&cli),
